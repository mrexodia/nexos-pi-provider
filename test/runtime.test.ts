...
for (const [field, wireId] of [
  ["nexos_model_id", "12345678-1234-1234-1234-123456789abc"],
  ["nexos_model_id", "auto-code"],
  ["id", "auto-code"],
] as const) test(`pi login/logout synchronizes the picker for ${field}=${wireId}`, async () => {
  ...
  try {
    ...
    runtime.registerNativeProvider(nexosProvider({ envKey: () => undefined, fetcher: async () => Response.json({ data: [{
      name: "Runtime Model", [field]: wireId,
    ...
    assert.equal(models[0].samplingParams?.model, wireId);
